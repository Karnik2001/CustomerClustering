...
malldf <- sample_frac(malldf,0.6)
mall.index <- as.numeric(rownames(malldf))
mall.test <- mall[-mall.index,]
head(malldf)
View(malldf)

# determine the data types in the data frame
str(malldf)
...
mall.subset3 <- as.data.frame(malldf[,c("Age","Spending Score")])
summary(mall.subset3)
...
mall.subset3 <- as.data.frame(scale(mall.subset3))
summary(mall.subset3)

# create plots of the elbow method
set.seed(102)
...
set.seed(102)
fviz_nbclust(mall.subset3, kmeans, method = "wss") + labs(subtitle = "Elbow method")

# create plots of the silhouette method
...
fviz_nbclust(mall.subset3,kmeans, method = "silhouette")+ labs(subtitle = "Silhouette method")

# create plots of the gap statistics method
set.seed(123)
...
fviz_nbclust(mall.subset3, kmeans, nstart = 25, method = "gap_stat", nboot = 50) + labs(subtitle = "Gap Statistic Method")

# create kmeans datasets to determine the number of clusters in each column
set.seed(145)
...
kmean.simple2 <- kmeans(mall.subset3, centers=2, iter.max = 30, nstart = 100)
mall.subset3$cluster <- factor(kmean.simple2$cluster)
summary(mall.subset3)
...
ggplot(data=mall.subset3, aes(x = Age, y= `Spending Score`, colour=cluster))+geom_point()+geom_point(data=as.data.frame(kmean.simple2$centers), 
color ="black", size=4, shape =17)

# plot the internal cluster validation of the kmeans subsets
...
E <- daisy(mall.subset3)
plot(silhouette(kmean.simple1$cluster, E), col = 1:2, border = NA)
...
stats1 <- cluster.stats(d = dist(malldf), species, kmean.simple1$cluster)

species <- as.numeric(kmean.simple2$cluster)
stats2 <- cluster.stats(d = dist(malldf), species, kmean.simple2$cluster)

# calculate the corrected Rand index
stats$corrected.rand
stats1$corrected.rand
stats2$corrected.rand
